...
def calculate_recall_quantities(manager, nr, T_recall, T_cue, remove=0.009, reset=True, empty_history=True,
                                steady=True, plasticity_on=False):
    n_seq = nr.shape[0]
    I_cue = nr[0]
...
def calculate_patterns_timings(manager, winning_patterns, remove=0):
    """
...
def calculate_probability_theo(Tp, Tstart, Ttotal, tau_z):
    ...
    p = Tp - tau_z * M * np.exp(-(Ttotal - Tp) / tau_z)
    return p / Ttotal
...
def joint_prob(T1, Ts, T2, Tt, tau1, tau2):
    """
    Calcualtes the joint probability of unit 1 and 2.
    :param T1:The time that the unit 1 remained activated (training time)
    :param Ts: The time at which the second unit becomes activated
    ...
    M1 = 1 - np.exp(-T1 / tau1)
    M2 = 1 - np.exp(-T2 / tau2)
    decay = np.exp(-Ts / tau1)

    aux1 = M1 * tau1 * decay * (1 - np.exp(-T2/tau1))

    aux2 = M1 * tau_p * decay * (1 - np.exp(-T2/tau_p))

    aux3 = M1 * M2 * tau_p * decay * np.exp(-T2 / tau1)
